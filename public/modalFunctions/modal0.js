function Pop() {
  ...
  let createPopUp = function () {
    ...
    <path d="M23 23.18L24.8 30.94L18 26.84L11.2 30.94L13 23.2L7 18.02L14.92 17.34L18 10.04L21.08 17.32L29 18L23 23.18ZM18 4.38L32 10.6V20C32 29.04 26.04 37.38 18 39.86C9.96 37.38 4 29.04 4 20V10.6L18 4.38ZM18 0L0 8V20C0 31.1 7.68 41.48 18 44C28.32 41.48 36 31.1 36 20V8L18 0Z" fill="white"/>
      ...
      mainDiv.className += ` relative z-10  w-[${content.size}] min-w-[400px]`

      mainDiv.innerHTML += ` <div class="flex min-w-[500px] relative shadow-[0_5px_30px_0px_rgba(0,0,0,0.15)]  bg-white  rounded-[30px]">
        <div class="flex flex-col items-center justify-center w-1/2 my-[60px]">
          <div class="flex flex-col items-center justify-center max-w-[300px]">
          <span class="font-semibold text-3xl leading-9 "> ${content.area.header}</span>
          <span class="text-base leading-[18px] mt-[10px]">
          ${content.area.description.description1}
          </span>
          <input
            id="input1"
            placeholder="${content.area.input.input1}"
            class="h-9 rounded-[8px] border-position-block-border border px-3 py-[10px] mt-[30px] focus-visible:outline-none"
          />
          <input
            id="input2"
            placeholder="${content.area.input.input2}"
            class="h-9 rounded-[8px] border-position-block-border border px-3 py-[10px] mt-[15px] focus-visible:outline-none"
          />
  
          <button
            id="continueButton"
            type="button"
            class=" rounded-lg mt-[15px] py-[10px] bg-primary-purple text-white w-full bg-[${content.color}] text-[${content.textColor}]"
          >
          ${content.area.button.button1} 
          </button>
          <span class='text-[10px] mt-[15px] mr-auto'>${content.area.description.description2}</span>
          </div>
        </div>
        <div class='w-1/2 inline-flex'>
          <img class="rounded-r-[30px]" src="${content.imageUrl}"/>
        </div>
        <button
      type="button"
      id="closeButton"
    ...
    <img src="http://localhost:3000/modalClose.png">
      ...
      var codeInput2 = document.getElementById('input2')
      codeInput2.addEventListener('input', function (e) {
        content.area.input.input2 = e.target.value
      })
  ...
  this.init = function (param) {
    ...
    if (typeof param === 'object') {
      ...
      if (param.area.description.description2)
        content.area.description.description2 = param.area.description.description2
      if (param.area.input.input1)
        content.area.input.input1 = param.area.input.input1
      if (param.area.input.input2)
        content.area.input.input2 = param.area.input.input2
      ...
      if (typeof window != 'undefined') {
        ...
        if ((param.desktop && param.mobile) || !param.isOpenVisitorDevice) {
          ...
          if (
            !param.isOpenXSeconds &&
            !param.isOpenXScroll &&
            !param.isOpenTrafficSource &&
            !param.isOpenBrowserLanguage
          ) {
            createPopUp()
          }
        }
